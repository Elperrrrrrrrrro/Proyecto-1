...
public class ConsolaEmpleado extends ConsolaUsuario {
	...
	private void registrarVentaCliente() {
		...
		int idMesa = leerEntero("Id mesa: ");
		agregarProductosMesa(String.valueOf(idMesa));
		...
		try {
			sistema.registrarVenta(idMesa, LocalDateTime.now(), cliente, propina, null, juegos, descuentoCompartido);
	...
	private void registrarVentaEmpleado() {
		...
		ArrayList<JuegoMesa> juegos = leerJuegosVenta();
		try {
			sistema.registrarVenta(null, LocalDateTime.now(), empleado, 0.0, productosExtras, juegos, false);
